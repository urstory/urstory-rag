...
```
GET  /api/health                    # 헬스체크
GET  /api/system/status             # 시스템 상태 (DB, ES, OpenAI API 연결, 모델 로드 상태)
POST /api/system/reindex-all        # 전체 재인덱싱 (비동기)
GET  /api/system/tasks/{task_id}    # 비동기 작업 상태 확인 (Celery 태스크)
```
...
```json
{
  ...
  "top_k": 5,
  "search_mode": "hybrid",       // hybrid | vector | keyword | cascading
  "hyde_enabled": true,
...
GET   /api/settings/models          # 사용 가능한 모델 목록 (OpenAI + 대안 API 모델)
```
...
```json
{
  "chunking": {
    ...
    "contextual_chunking_model": "gpt-4.1-mini"
  },
  "embedding": {
    "provider": "openai",
...
POST /api/evaluation/datasets                   # 데이터셋 생성 (JSON body)
...
```json
// POST /api/evaluation/datasets (JSON body, FormData 아님)
{
  "name": "v1 품질 평가 데이터셋",
  "items": [
    {
      "question": "연차 신청 절차가 어떻게 되나요?",
      "ground_truth": "연차 신청은 사내 포털에서..."
    }
  ]
}
```
